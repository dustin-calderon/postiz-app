...
export async function postWorkflowV106({
  ...
  // v1.0.6 — the webhook must also fire when a post FAILS, not only on success.
  // Without this, an external consumer (n8n) can never tell a failure from a
  // post that simply has not been published yet.
  ...
  for (let i = 0; i < postsList.length; i++) {
    ...
    if (postsResults.length === before) {
      ...
      // v1.0.6 — this path could previously exit with the post still in QUEUE:
      // if every attempt failed with `refresh_token` and the refresh succeeded,
      // `continue` runs without ever touching the state.
      await changeState(
        postsList[0].id,
        'ERROR',
        'All retries exhausted without a successful post',
        postsList
      );
      return await notifyFailure();
    }
  ...
  // v1.0.6 — was `postsResults[0].postId`, which is the SOCIAL NETWORK's id.
  // `getPostByForWebhookId` looks up `Post.id`, so the payload always came back
  // as an empty array. It must be the internal post id.
  await sendWebhooks(
    postsList[0].id,
    post.organizationId,
    post.integration.id
