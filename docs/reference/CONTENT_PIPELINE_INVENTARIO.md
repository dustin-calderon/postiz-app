...
| `postiz-media-huerfana.py` | `/opt/homeserver/scripts/` | Libera cada noche la media del pipeline que ya no es de nadie ([MEDIA_CLEANUP_PIPELINE.md](../architecture/MEDIA_CLEANUP_PIPELINE.md)). Cron a las 03:10, con aviso al bus si falla; `--seco` dice qué liberaría |
